...
import type { AmcPlanItem } from "@/lib/site/types";
...
export function AMCPlans({ plans, callHref }: AMCPlansProps) {
  return (
    <section id="amc" className="py-20 lg:py-28 bg-gradient-to-b from-blue-950 to-blue-900 relative overflow-hidden" aria-labelledby="amc-heading">
      <div
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection className="text-center mb-16">
          ...
          <h2 id="amc-heading" className="text-3xl sm:text-4xl lg:text-5xl font-black text-white">
            AMC{" "}
            <span className="bg-gradient-to-r from-cyan-400 to-teal-300 bg-clip-text text-transparent">Protection Plans</span>
          </h2>
          <p className="text-blue-200/70 text-lg mt-4" lang="ta">
            ...
            <AnimatedSection key={plan.name} delay={i * 100}>
              <div
                className={`relative rounded-3xl overflow-hidden flex flex-col h-full ${
                  plan.highlight ? "ring-2 ring-cyan-400 ring-offset-2 ring-offset-blue-900 md:scale-105" : ""
                }`}
              >
                {plan.highlight ? (
                  <div className="absolute top-0 left-0 right-0 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-xs font-bold text-center tracking-widest uppercase z-10">
                    Most Popular
                  </div>
                ) : null}
                ...
                <div className="bg-white/10 backdrop-blur-sm flex-1 p-6 space-y-3">
                  {plan.features.map((f) => (
                    <div key={f} className="flex items-center gap-3 text-white">
                      <CheckCircle className="w-4 h-4 text-cyan-400 flex-shrink-0" aria-hidden />
