...
import type { BookingStep, WhyChoosePoint } from "./types";
          ...
          <AnimatedSection>
            ...
            <div className="mt-8 grid grid-cols-2 gap-4">
              {points.map((item) => (
                <div
                  key={item.title}
                  className="flex gap-3 bg-white rounded-2xl p-4 shadow-sm border border-slate-100 hover:border-cyan-200 hover:shadow-md transition-all"
                >
          ...
          <AnimatedSection delay={200}>
            <div className="relative">
              <div className="bg-gradient-to-br from-blue-950 to-cyan-900 rounded-3xl p-8 text-white relative overflow-hidden">
                <div className="absolute top-0 right-0 w-48 h-48 bg-cyan-500/10 rounded-full -translate-y-1/2 translate-x-1/2" />
                ...
                <div className="space-y-4 relative z-10">
                  {bookingSteps.map((item) => (
                    <div key={item.step} className="flex gap-4 bg-white/10 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mt-6 relative z-10">
                  <a
                    href={callHref}
                    className="flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-xl font-bold text-sm hover:scale-105 transition-transform shadow-lg"
                  >
